...
(function (global) {
  ...
  function initReveal() { const items=document.querySelectorAll('.reveal'); if(!items.length)return; const reveal=()=>{const vh=global.innerHeight;items.forEach(el=>{if(el.getBoundingClientRect().top<vh-150)el.classList.add('active');});};global.addEventListener('scroll',reveal,{passive:true});reveal(); }
  ...
  function vatBreakdown(gross){const c=LUERI.company;const on=!!c.vatRegistered;const vat=on?(Number(gross)*c.vatRate/(1+c.vatRate)):0;return{vatRegistered:on,rate:c.vatRate,vat:vat,net:Number(gross)-vat,gross:Number(gross)};}

...
type DeliveryState = {
  step: "IDLE" | "TRACKING" | "DELIVERY_TYPE" | "MULTI_TRIP_COUNT" | "PICKUP" | "DROPOFF" | "PARCEL" | "TIME" | "NAME" | "PHONE" | "EMAIL" | "REVIEW";
  delivery_type?: "one_off" | "round_trip" | "multi_trip";
  trip_count?: number;
  pickup?: string;
  dropoff?: string;
  details?: string;
...
function deliveryTypeLabel(locale: string, type?: string) {
  const labels: Record<string, Record<string,string>> = {
    en:{one_off:"One-off delivery",round_trip:"Round-trip delivery",multi_trip:"Multi-trip / rebook"},
    sw:{one_off:"Delivery ya mara moja",round_trip:"Delivery ya kwenda na kurudi",multi_trip:"Multi-trip / rebook"},
    fr:{one_off:"Livraison ponctuelle",round_trip:"Livraison aller-retour",multi_trip:"Multi-trajets / rebooking"},
    es:{one_off:"Entrega puntual",round_trip:"Entrega de ida y vuelta",multi_trip:"Multi-viaje / rebooking"},
    ar:{one_off:"توصيل لمرة واحدة",round_trip:"توصيل ذهاب وعودة",multi_trip:"رحلات متعددة / إعادة حجز"},
    pt:{one_off:"Entrega única",round_trip:"Entrega de ida e volta",multi_trip:"Multi-viagem / rebooking"},
    zh:{one_off:"单次配送",round_trip:"往返配送",multi_trip:"多次配送 / 重新预约"}
  };
  return labels[locale]?.[String(type||"one_off")] ?? labels.en[String(type||"one_off")] ?? "One-off delivery";
}
function fill(template: string, state: DeliveryState, locale = "en") {
  return template.replace("{delivery_type}", deliveryTypeLabel(locale, state.delivery_type))
    .replace("{trip_count}", String(state.trip_count ?? 1))
    .replace("{pickup}", state.pickup ?? "").replace("{dropoff}", state.dropoff ?? "")
    .replace("{details}", state.details ?? "").replace("{time}", state.preferred_time ?? "")
    .replace("{name}", state.customer_name ?? "").replace("{phone}", state.customer_phone ?? "")
    .replace("{email}", state.customer_email ?? "");
}
...
async function handle(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (state.step === "IDLE" && (topicIntent === "BOOK" || isBookingIntent(message))) {
      state = { step: "DELIVERY_TYPE", member_id: state.member_id ?? null };
      reply = flow(validLocale, "deliveryType");
    }
    ...
    if (!reply && state.step !== "IDLE") {
      switch (state.step) {
        case "DELIVERY_TYPE": {
          ...
          const type = /^(1|one|one-off|one off|single|mara moja|moja|une|una|puntual|لمرة واحدة|单次)$/i.test(choice) ? "one_off" : /^(2|round|round-trip|round trip|ida na kurudi|aller-retour|ida y vuelta|ذهاب وعودة|往返)$/i.test(choice) ? "round_trip" : /^(3|multi|multi-trip|multi trip|rebook|rebooking|safari nyingi|multi-trajets|multi-viaje|رحلات متعددة|多次)$/i.test(choice) ? "multi_trip" : null;
          if (!type) { reply = flow(validLocale, "deliveryType"); break; }
          state.delivery_type = type as DeliveryState["delivery_type"];
          state.trip_count = type === "multi_trip" ? undefined : 1;
          if (type === "multi_trip") { state.step = "MULTI_TRIP_COUNT"; reply = flow(validLocale, "multiTripCount"); }
          else { state.step = "PICKUP"; reply = flow(validLocale, "pickup"); }
          break;
        }
        case "MULTI_TRIP_COUNT": {
          const count = Number.parseInt(message.trim(), 10);
          if (!Number.isInteger(count) || count < 2 || count > 100) { reply = flow(validLocale, "multiTripCount"); break; }
          state.trip_count = count;
          state.step = "PICKUP";

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!reply) {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model: MODEL, max_tokens: 300, system: augmentedSystemPrompt, system: SYSTEM_PROMPT + "\n\nACTIVE LOCALE: " + (LOCALE_INSTRUCTIONS[validLocale] ?? LOCALE_INSTRUCTIONS.en),\n    messages: [...history, { role: "user", content: message }] }) });

...
function toLocalPhone(v:string){const d=v.replace(/\D/g,"");return d.startsWith("254")&&d.length===12?"0"+d.slice(3):d}
...
async function handle(req:Request):Promise<Response>{
 ...
 try{
  ...
  const customerName=String(body?.customer_name??"").trim(),phone=toLocalPhone(String(body?.phone??body?.customer_phone??"").trim());
  ...
  if(parcelPhotoPath&&!/^\d{4}-\d{2}-\d{2}\/[0-9a-f-]{36}\.(jpg|png|webp)$/.test(parcelPhotoPath))return json({error:"Invalid parcel photo reference."},400);
  if(pickup.length>300||dropoff.length>300||details.length>1500||customerName.length>120)return json({error:"One of the fields is too long."},400);
  ...
  const {data:booking,error:bookingInsertError}=await supabaseAdmin.from("bookings").insert({customer_name:customerName,customer_email:customerEmail,phone,pickup,dropoff,details:details||null,preferred_time:preferredTime,member_id:memberId,parcel_photo_path:parcelPhotoPath,status:"pending_payment",quoted_amount_kes:amount,reference:internalReference}).select("id").single();
  ...
  try{
   ...
   const orderRes=await fetch(`${BASE_URL}/Transactions/SubmitOrderRequest`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({id:internalReference,currency:"KES",amount,description:`Lueri Delivery: ${pickup} to ${dropoff}`,callback_url:`${SITE_URL}/payment-callback.html?payment=complete&type=delivery`,notification_id:IPN_ID,billing_address:{email_address:customerEmail||"booking@lueriinternational.com",phone_number:normalizePhone(phone),country_code:"KE",first_name:names.first_name,last_name:names.last_name}})});
   ...
   const {error:trackErr}=await supabaseAdmin.from("payments").update({pesapal_tracking_id:trackingId}).eq("id",payment.id);if(trackErr)console.error("could not save tracking id on payment",trackErr);

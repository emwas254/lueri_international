...
function renderPendingPhotoAction(){
  ...
  async function sendMessage(text,topicIntent='',imageData=''){if(busy)return;const input=document.getElementById('lucy-input');const message=imageData?(locale==='sw'?'📷 Picha ya kifurushi imeambatishwa':locale==='fr'?'📷 Photo du colis jointe':locale==='es'?'📷 Foto del paquete adjunta':locale==='ar'?'📷 تم إرفاق صورة الطرد':locale==='pt'?'📷 Foto do pacote anexada':locale==='zh'?'📷 已附上包裹照片':'📷 Parcel photo attached'):String(text??input?.value??'').trim();if(!message)return;if(input)input.value='';addMessage(message,'user');busy=true;const send=document.getElementById('lucy-send');if(send)send.disabled=true;const typing=addMessage(locale==='zh'?'正在输入…':locale==='sw'?'Inaandika…':locale==='fr'?'Saisie…':locale==='es'?'Escribiendo…':locale==='ar'?'يكتب الآن…':locale==='pt'?'Digitando…':'Typing…','bot',true);
    try{const res=await fetch(`${API_BASE}/lucy-chat`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message,locale,topic_intent:topicIntent,delivery_state:deliveryState,history:history.slice(-9),...(imageData?{image_data:imageData}:{})})});const data=await res.json().catch(()=>({}));typing?.remove();if(!res.ok)throw new Error(data?.error||'Request failed');const reply=String(data?.reply||'');if(reply)addMessage(reply,'bot');if(data?.delivery_state&&typeof data.delivery_state==='object')deliveryState=data.delivery_state;renderParcelActions();clearPendingPhoto();history.push({role:'user',content:message});if(reply)history.push({role:'assistant',content:reply});history=history.slice(-20);saveSession();if(data?.action==='INITIATE_PAYMENT')document.dispatchEvent(new CustomEvent('lueri-lucy-payment-ready',{detail:data}));}
    catch(err){typing?.remove();if(imageData)savePendingPhoto(imageData);addMessage(locale==='zh'?'抱歉，Lucy 暂时无法连接。照片已保留在本次会话中，恢复连接后可重试。':locale==='sw'?'Samahani, Lucy hawezi kuunganishwa kwa sasa. Picha imehifadhiwa kwenye kikao hiki; unaweza kujaribu tena muunganisho ukirejea.':locale==='fr'?'Désolée, Lucy ne peut pas se connecter. La photo est conservée dans cette session et pourra être renvoyée après rétablissement de la connexion.':locale==='es'?'Lo siento, Lucy no puede conectarse. La foto se conserva en esta sesión y podrá reenviarse cuando vuelva la conexión.':locale==='ar'?'عذراً، لا تستطيع لوسي الاتصال الآن. تم الاحتفاظ بالصورة في هذه الجلسة ويمكن إعادة إرسالها بعد عودة الاتصال.':locale==='pt'?'Desculpe, a Lucy não consegue ligar-se agora. A foto foi mantida nesta sessão e poderá ser reenviada quando a ligação voltar.':locale==='zh'?'抱歉，Lucy 暂时无法连接。照片已保留在本次会话中，恢复连接后可重试。':'Sorry, Lucy cannot connect right now. The photo is kept in this session and can be retried when the connection returns.','bot');locale==='sw'?'Samahani, Lucy hawezi kuunganishwa kwa sasa. Jaribu tena baadaye.':locale==='fr'?'Désolée, Lucy ne peut pas se connecter pour le moment. Réessayez plus tard.':locale==='es'?'Lo siento, Lucy no puede conectarse ahora. Inténtalo de nuevo más tarde.':locale==='ar'?'عذراً، لا تستطيع لوسي الاتصال الآن. يرجى المحاولة لاحقاً.':locale==='pt'?'Desculpe, a Lucy não consegue conectar-se agora. Tente novamente mais tarde.':'Sorry, Lucy cannot connect right now. Please try again later.','bot');console.error('Lucy chat error',err);}
